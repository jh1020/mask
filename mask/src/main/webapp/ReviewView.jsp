...
<body>
	<%
		...
		if(bbsID == 0) {
			...
			script.println("location.href='ReviewBoard.jsp'");
			script.println("</script>");
		}
		
		//유효한 글이라면 구체적인 정보를 'bbs'라는 인스턴스에 담는다
		Review bbs = new ReviewDAO().getBbs(bbsID);
	...
	<div class="container">
		<div class="row">
			...
			<a href="ReviewBoard.jsp" class="btn btn-primary">목록</a>
			
			<!-- 해당 글의 작성자가 본인이라면 수정과 삭제가 가능하도록 코드 추가 -->
			<%
				if(userId != null && userId.equals(bbs.getUserId())){
			%>
					<a href="ReviewUpdate.jsp?bbsID=<%= bbsID %>" class="btn btn-primary">수정</a>
					<a onclick="return confirm('정말로 삭제하시겠습니까?')" href="ReviewDeleteAction.jsp?bbsID=<%= bbsID %>" class="btn btn-primary">삭제</a>
			<%
				}
			%>
		</div>
